            ...
            for a in adja:
                if a.split(':')[0] == da:
                    if curdate == a.split(':')[1]:
                        ...
                        self.buy(size=(((float(cerebro.broker.getcash()) / 10)/(float(adja[a]['c'])*float(adja[a]['price']))/(1/float(adja[a]['c']))) * float(adja[a]['c'])),data=self.getdatabyname(da), exectype=backtrader.Order.StopTrail, trailpercent=0.075)
            for d in adjd:
                if d.split(':')[0] == da:
                    if curdate == d.split(':')[1]:
                        ...
                        self.sell(size=(((float(cerebro.broker.getcash()) / 10)/(float(adjd[d]['c'])*float(adjd[d]['price']))/(1/float(adjd[d]['c']))) * float(adjd[d]['c'])),data=self.getdatabyname(da), exectype=backtrader.Order.StopTrail, trailpercent=0.075)
